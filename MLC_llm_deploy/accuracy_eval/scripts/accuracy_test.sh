...
HOST="${HOST:-localhost}"
PORT="${PORT:-8000}"
TEST_CASES="${TEST_CASES:-$EVAL_DIR/testcases/quick_check.json}"
...
prepare_test_data() {
    ...
    if [ ! -f "$TEST_CASES" ]; then
        ...
        cat > "$EVAL_DIR/testcases/quick_check.json" << 'EOF'
{
  "test_cases": [
...
EOF
        log_info "✓ 默认测试用例已创建: $EVAL_DIR/testcases/quick_check.json"
    else
